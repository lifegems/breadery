...
var SchedulesService = (function () {
    ...
    SchedulesService.prototype.getSavedScheduleID = function () {
        return appSettings.getString('ScheduleID', "001");
    };
    SchedulesService.prototype.getReadingProgress = function () {
        return appSettings.getString('ReadingProgress', "").split(",");
    };
    SchedulesService.prototype.saveScheduleID = function (ScheduleID) {
        return appSettings.setString('ScheduleID', ScheduleID);
    };
    SchedulesService.prototype.saveScheduleProgress = function (item) {
        var Progress = appSettings.getString('ReadingProgress', "").split(",");
        Progress.push(item.code);
        appSettings.setString('ReadingProgress', Progress.toString());
    };
    SchedulesService.prototype.removeScheduleProgress = function (item) {
        var aProgress = appSettings.getString('ReadingProgress', "").split(",");
        var i = aProgress.indexOf(item.code);
        if (i > -1) {
            aProgress = aProgress.splice(i, 1);
        }
        appSettings.setString('ReadingProgress', aProgress.toString());
    };
    SchedulesService.prototype.isReadingComplete = function (strReading) {
        var blIsComplete = false;
        var aProgress = appSettings.getString('ReadingProgress', "").split(",");
        for (var i = 0; i < aProgress.length; i++) {
            if (aProgress[i] === strReading) {
                blIsComplete = true;
            }
        }
        return blIsComplete;
    ...
    SchedulesService.prototype.formatAsString = function (intID) {
        var intNum = parseInt(intID);
        var strNum = "";
        if (intNum < 10) {
            strNum = "00" + intNum.toString();
        }
        else if (intNum < 100) {
            strNum = "0" + intNum.toString();
        }
        else {
