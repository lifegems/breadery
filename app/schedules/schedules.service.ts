...
export class SchedulesService {
   private rooturl = "https:/api.mlab.com/api/1/databases/lifegems/collections/Schedules";
   ...
   saveScheduleProgress(item) {
      let Progress = appSettings.getString('ReadingProgress', "").split(",");
      Progress.push(item.code);
      appSettings.setString('ReadingProgress', Progress.toString());
   }
   
   removeScheduleProgress(item) {
      let aProgress = appSettings.getString('ReadingProgress', "").split(",");
      let i = aProgress.indexOf(item.code);
      if (i > -1) {
   ...
   isReadingComplete(strReading) {
      let blIsComplete = false;
      let aProgress = appSettings.getString('ReadingProgress', "").split(",");
   ...
   formatAsString(intID) {
      let intNum = parseInt(intID);
      let strNum = "";
      if (intNum < 10) {
